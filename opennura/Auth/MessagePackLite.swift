...
enum MessagePackLite {
    ...
    private static func writeValue(_ value: Any?, to buffer: inout [UInt8]) {
        ...
        // Binary MUST be matched before [Any?]: a [UInt8] bridges to [Any?] via
        // `as`, so if the array case came first, byte payloads would be encoded
        // as a msgpack array of ints instead of a binary blob (the backend
        // rejects that with HTTP 500 on session/start_N).
    ...
    private static func readValue(_ data: [UInt8], _ offset: inout Int) -> Any? {
        ...
        case 0xcf:
            let value = readUInt64(data, &offset)
    ...
    private static func readMap(_ data: [UInt8], _ offset: inout Int, count: Int) -> [String: Any?] {
        var map: [String: Any?] = [:]
        // A length field can't legitimately exceed the bytes remaining (each
        // entry needs at least a key + value byte), so clamp it. This stops a
        // malicious/corrupted response with a huge count from spinning billions
        // of iterations or exhausting memory.
        let safeCount = min(max(count, 0), data.count - offset)
        for _ in 0..<safeCount {
            if offset >= data.count { break }
            let rawKey = readValue(data, &offset)
            // Always consume the value too, so a non-string key can't desync
    ...
    private static func readArray(_ data: [UInt8], _ offset: inout Int, count: Int) -> [Any?] {
        var list: [Any?] = []
        let safeCount = min(max(count, 0), data.count - offset)
        list.reserveCapacity(safeCount)
        for _ in 0..<safeCount {
            if offset >= data.count { break }
